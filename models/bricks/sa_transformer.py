"""Transformer implementation used by SA-DETR.

The encoder integrates TRRO after multi-scale deformable self-attention. The
decoder retains relation-aware query interaction from the Relation-DETR
baseline.
"""
...
from models.bricks.trro import TRRO


class SATransformer(TwostageTransformer):
    def __init__(
        self,
        encoder: nn.Module,
        decoder: nn.Module,
...
class SATransformerEncoderLayer(nn.Module):
    ...
    def _load_from_state_dict(
        ...
        """Map the historical TFM parameter path to the canonical TRRO path."""
        legacy_prefix = prefix + "tfm."
        canonical_prefix = prefix + "trro."
        for key in list(state_dict):
            if key.startswith(legacy_prefix):
                canonical_key = canonical_prefix + key[len(legacy_prefix):]
                if canonical_key not in state_dict:
                    state_dict[canonical_key] = state_dict[key]
                state_dict.pop(key)
    ...
    def forward(
        ...
        # Optimize the target-region representation at every feature level.
        query = self.trro(query, spatial_shapes)
...
class SATransformerDecoder(nn.Module):
...
class SATransformerDecoderLayer(nn.Module):
    def __init__(
        self,
        embed_dim=256,
        d_ffn=1024,
